...
internal class SocialController : Controller
{
  ...
  public async Task<IActionResult> GetFeed()
  {
    ...
    var dtos = messages.Select(m => new SocialMessageDto
    {
      ...
      AttachmentUrl = string.IsNullOrEmpty(m.AttachmentUrl) ? null : new Uri(m.AttachmentUrl, UriKind.RelativeOrAbsolute),
      CreatedAt = m.CreatedAt,
      AuthorId = m.AuthorId,
      AuthorAvatarUrl = m.Author?.Profile?.AvatarUrl ?? new Uri("about:blank"),
  ...
  public async Task<IActionResult> PostMessage([FromBody] PostMessageRequest request)
  {
    ...
    var dto = new SocialMessageDto
    {
      ...
      AttachmentUrl = string.IsNullOrEmpty(savedMessage.AttachmentUrl) ? null : new Uri(savedMessage.AttachmentUrl, UriKind.RelativeOrAbsolute),
      CreatedAt = savedMessage.CreatedAt,
      AuthorId = savedMessage.AuthorId,
      AuthorAvatarUrl = savedMessage.Author?.Profile?.AvatarUrl ?? new Uri("about:blank"),
